...
function configureBrowserLaunchArgs(engine, options) {
    engine.launchArguments.push('--disable-background-timer-throttling', // Disable timers being throttled in background pages/tabs
    ...
    '--disable-features=LazyFrameLoading', '--disable-features=DestroyProfileOnBrowserClose', // Disable the feature of: Destroy profiles when their last browser window is closed, instead of when the browser exits.
    ...
    '--disable-blink-features=AutomationControlled', '--disable-hang-monitor', '--disable-ipc-flooding-protection', // Some javascript functions can be used to flood the browser process with IPC. By default, protection is on to limit the number of IPC sent to 10 per second per frame.
