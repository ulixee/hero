...
describe('basic resource tests', () => {
    ...
    it('waits for resources by default since the previous command', async () => {
        ...
        const resources = await hero.waitForResources({ type: 'Fetch' });
        expect(resources).toHaveLength(1);
        await expect(hero.waitForResource({ type: 'Fetch' }, { sinceCommandId })).resolves.toStrictEqual(resources[0]);
        ...
        await expect(hero.waitForResource({ type: 'Fetch' }, { sinceCommandId })).resolves.toStrictEqual(resources2[0]);
        let counter = 0;
    ...
    it('waits for a resource loaded since a previous command id', async () => {
        ...
        for (let i = 0; i <= 4; i += 1) {
            const elem = hero.document.querySelector('a');
            await hero.click(elem);
            const resources = await hero.waitForResources({ type: 'Fetch' }, { sinceCommandId: lastCommandId });
    ...
    it('collects resources for extraction', async () => {
        ...
        hero_testing_1.Helpers.needsClosing.push(hero1);
        {
            ...
            const coreSession1 = await hero1[internal_1.InternalPropertiesSymbol].coreSessionPromise;
            const { resourceMeta, coreTabPromise } = resource[internal_1.InternalPropertiesSymbol];
        ...
        // Test that we can load a previous session too
        {
            ...
            const coreSession2 = await hero2[internal_1.InternalPropertiesSymbol].coreSessionPromise;
