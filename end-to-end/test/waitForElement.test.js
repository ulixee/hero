...
<script>
    ...
    it('times out waiting for an element', async () => {
        ...
        await expect(tab.querySelector('a#notthere').$waitForExists({ timeoutMs: 500 })).rejects.toThrow(/Timeout waiting for element to exist/);
    });
...
<script>
    ...
    it('will yield an error for a bad querySelector', async () => {
        ...
        await expect(tab.querySelector('button-title="test"').$waitForVisible()).rejects.toThrow('valid selector');
    });
...
<script>
    setTimeout(function() {
      ...
      document.querySelector('ul').append(child);
        ...
        await expect(tab.querySelector('ul').children[2].$waitForVisible({
            timeoutMs: 5e3,
        })).resolves.toBeTruthy();
    });
...
async function createSession(options) {
    const hero = new hero_testing_1.Hero(options);
    hero_testing_1.Helpers.needsClosing.push(hero);
    return { tab: hero.activeTab, hero };
}
//# sourceMappingURL=waitForElement.test.js.map