...
(0, index_1.testIfNotOnGithubMac)('should get the correct webgl vendor from a nested srcdoc iframe', async () => {
    const agent = pool.createAgent({
        logger,
    });
    index_1.Helpers.needsClosing.push(agent);
...
describe('Proxy detections', () => {
 ...
 try {
    ...
    (0, index_1.testIfNotOnGithubMac)('cannot detect a proxy of args passed into a proxied function', async () => {
        const agent = pool.createAgent({
            logger,
        });
        index_1.Helpers.needsClosing.push(agent);
