...
class FramesManager extends eventUtils_1.TypedEventEmitter {
    ...
    constructor(page, devtoolsSession) {
        ...
        this.events.on(this.websocketSession, 'message-received', this.onWebsocketSessionMessageRecieved);
    }
    ...
    async addPageCallback(name, onCallback) {
        const callbacks = this.pageCallbacks.get(name) ?? [];
        if (callbacks.length === 0)
            this.pageCallbacks.set(name, callbacks);
    ...
    async getDefaultContextIdForFrameId(opts) {
        ...
        const contextId = new Resolvable_1.default(3e3);
    ...
    async onWebsocketSessionMessageRecieved(event) {
