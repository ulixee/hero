...
#### Properties
 ...
 |   |   | 
 | --- | --- | 
 | `audioTracks` | `autoplay`
`buffered` | `controls`
`controlsList` | `crossOrigin`
`currentSrc` | `currentTime`
`defaultMuted` | `defaultPlaybackRate`
`disableRemotePlayback` | `duration`
`ended` | `error`
`loop` | `mediaKeys`
`muted` | `networkState`
...
#### Methods
 ...
 |   |   | 
 | --- | --- | 
 | `addTextTrack()` | `canPlayType()`
`captureStream()` | `load()`
`pause()` | `play()`
`setMediaKeys()` | `setSinkId()`
