...
const DomState_1 = require("@ulixee/hero/lib/DomState");
let koaServer;
beforeAll(async () => {
...
test('does not allow nested "waitFor" commands', async () => {
    const hero = await openBrowser('/');
    await expect(hero.activeTab.waitForState({
        all(assert) {
...
test('can test for state across redirects', async () => {
    ...
    const state = new DomState_1.default({
        all(assert) {
...
test('surfaces errors in assertions', async () => {
    ...
    await expect(hero.activeTab.waitForState(domState)).rejects.toThrow('test.includes is not a function');
    await expect(hero.activeTab.validateState(domState)).rejects.toThrow('test.includes is not a function');
});
