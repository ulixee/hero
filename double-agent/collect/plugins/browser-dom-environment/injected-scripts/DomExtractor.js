function DomExtractor(selfName, pageMeta = {}) {
    ...
    function debugPath(path) {
        ...
        stallTimer = setTimeout(() => {
            if (!lastDebugPath || stallPostedForPath === lastDebugPath)
                return;
    ...
    async function getFunctionDetails(value, obj, key, type, path) {
        ...
        if (type === 'function') {
            ...
            try {
                const isSvgPrototypeMethod = path.startsWith(`${selfName}.SVG`) && path.includes('.prototype.');
                if (!isSvgPrototypeMethod &&
