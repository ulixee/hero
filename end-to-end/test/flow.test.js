...
test('will trigger a flow handler when an error occurs', async () => {
  ...
  <script type="text/javascript">
    ...
    const spy = jest.fn();
    await hero.registerFlowHandler('flow', assert => {
...
test('can handle multiple simultaneous handlers', async () => {
    ...
    await hero.registerFlowHandler('flow', assert => assert(hero.querySelector('#test').$isVisible), () => Promise.resolve());
    await hero.registerFlowHandler('flow2', assert => assert(hero.url, 'https://test.com'), () => Promise.resolve());
    await hero.registerFlowHandler('flow3', assert => {
        assert(hero.querySelector('h3'));
        assert(hero.url, 'https://test2.com');
    }, () => Promise.resolve());
    await hero.registerFlowHandler('flow', assert => assert(hero.findResource({ httpRequest: { statusCode: 403 } })), () => Promise.resolve());
...
<body>
  ...
  <script type="text/javascript">
    ...
    it('can restart a flow command block', async () => {
        ...
        await hero.registerFlowHandler('FormInvisible', assert => {
            assert(hero.querySelector('.ready').$isVisible, false);
        }, async (error) => {
            flowHandlerSpy();
            expect(error).not.toBeTruthy();
  ...
  let counter = 0;
    function keyup() {
      ...
      if (field.value === 'tes' && counter === 0) {
        ...
        await hero.registerFlowHandler('Refocus', assert => {
...
<body>
  ...
  function closeCookiePrompt() {
    dialog.style.display = 'none'
        ...
        await hero.registerFlowHandler('CookiePrompt', assert => {
