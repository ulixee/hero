...
export async function validateHostRequirements(browserEngine: IBrowserEngine): Promise<void> {
  const isWindows64 = os.platform() === 'win32' && os.arch() === 'x64';
  const isLinux = os.platform() === 'linux';

  if (!isLinux && !isWindows64) return;

  const installDirectory = path.dirname(browserEngine.executablePath);
  const missingDeps = await findAllMissingDependencies(installDirectory);
  const isDebian = isLinux && (await isDebianFlavor());
  if (!missingDeps.size) {
    await DependencyInstaller.markValidated(browserEngine);
    return;
  }

  const { name, fullVersion } = browserEngine;
  ...
  if (isWindows64) {
    ...
    await DependencyInstaller.appendAptInstallNeeded(browserEngine);
