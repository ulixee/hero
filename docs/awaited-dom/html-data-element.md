# [AwaitedDOM](../basic-client/awaited-dom) <span>/</span> HTMLDataElement

<div class='overview'>The <strong><code>HTMLDataElement</code></strong> interface provides special properties (beyond the regular <code>HTMLElement</code> interface it also has available to it by inheritance) for manipulating <code>&lt;data&gt;</code> elements.</div>

## Properties

### elem.value <div class="specs"><i>W3C</i></div> {#value}

Is a `string` reflecting the <code>value</code> HTML attribute, containing a machine-readable form of the element's value.
