...
const MitmRequestAgent_1 = require("@ulixee/unblocked-agent-mitm/lib/MitmRequestAgent");
...
describe('UserProfile cookie tests', () => {
    it('should be able to save and restore cookies', async () => {
        ...
        const tab2 = Session_1.default.getTab(meta2);
        hero_testing_1.Helpers.needsClosing.push(tab2.session);
        ...
        const tab3 = Session_1.default.getTab(meta3);
        hero_testing_1.Helpers.needsClosing.push(tab3.session);
    ...
    it('should track cookies from other domains', async () => {
        ...
        }
        {
            ...
            const session = tab.session;
            hero_testing_1.Helpers.needsClosing.push(session);
            const dlfCookies = (0, utils_1.createPromise)();
            const sameCookies = (0, utils_1.createPromise)();
            session.mitmRequestSession.interceptorHandlers.push({
                urls: ['https://ulixee.org/*'],
                types: [],
                handlerFn(url, type, request, response) {
...
localStorage.setItem('Test1', 'value1');
    ...
    it('should not make requests to end sites during profile "install"', async () => {
        const mitmSpy = jest.spyOn(MitmRequestAgent_1.default.prototype, 'request');
...
<script>
    ...
    openDBRequest.onupgradeneeded = function() {
        ...
        });
        {
            ...
            await tab.waitForLoad('PaintingStable');
            await hero_testing_1.Helpers.waitForElement(['document', ['querySelector', 'body.ready']], tab.mainFrameEnvironment);
            const profile = await tab.session.exportUserProfile();
    ...
    indexQuery.onsuccess = function({ target }) {
      ...
      if (completed.size === 2) {
        ...
        }
        {
            ...
            await tab.waitForLoad('PaintingStable');
            await hero_testing_1.Helpers.waitForElement(['document', ['querySelector', 'body.ready']], tab.mainFrameEnvironment);
            const recordsJson = await tab.execJsPath([
                'document',
            ...
            const indexLookupJson = await tab.execJsPath([
                'document',
