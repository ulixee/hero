...
export default function install(browserEngine: IBrowserEngineConfig) {
  installEngineWithProgress(browserEngine).catch(error => {
    console.log('ERROR installing browser engine', error);
    process.exit(1);
  });
}

export async function installEngineWithProgress(engineConfig: IBrowserEngineConfig): Promise<void> {
  if (shouldSkipDownload()) return;

  const { fullVersion, name, executablePathEnvVar } = engineConfig;
